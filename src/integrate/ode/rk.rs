    ...
    F: Fn(f64, &[f64]) -> Vec<f64>,
{
    ...
    match options.method {
        ...
        ODEMethod::DOP853 => dop853(f, t_span, y0, options),
    }
}
...
fn rk23<F>(f: F, t_span: [f64; 2], y0: &[f64], options: &ODEOptions) -> IntegrateResult<ODEResult>
where
    F: Fn(f64, &[f64]) -> Vec<f64>,
{
    let [t_start, t_end] = t_span;
...
/// Dormand-Prince 8(5,3) method (DOP853).
///
/// 12-stage explicit Runge-Kutta method of order 8 with embedded 5th order
/// error estimator. Best for high-accuracy requirements on smooth problems.
fn dop853<F>(f: F, t_span: [f64; 2], y0: &[f64], options: &ODEOptions) -> IntegrateResult<ODEResult>
where
    F: Fn(f64, &[f64]) -> Vec<f64>,
{
    // Delegate to the full DOP853 implementation
    super::dop853::dop853(f, t_span, y0, options)
}
...
mod tests {
    ...
    fn test_rk23() {
        ...
        let result = solve_ivp(|_t, y| vec![-y[0]], [0.0, 2.0], &[1.0], &options).unwrap();
