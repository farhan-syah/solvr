...
//!
//! Implements the [`RootFindingAlgorithms`] trait for CPU runtime.
//! All implementations delegate to the generic implementations in `impl_generic/roots/`.

use crate::optimize::impl_generic::{broyden1_impl, levenberg_marquardt_impl, newton_system_impl};
use crate::optimize::roots::{RootFindingAlgorithms, RootOptions, RootTensorResult};
...
mod tests {
    ...
    fn linear_system<R: numr::runtime::Runtime>(x: &Tensor<R>) -> Result<Tensor<R>> {
        let data: Vec<f64> = x.to_vec();
        let residuals = vec![data[0] + data[1] - 3.0, 2.0 * data[0] - data[1]];
        Ok(Tensor::from_slice(&residuals, &[2], x.device()))
    }

    fn quadratic_system<R: numr::runtime::Runtime>(x: &Tensor<R>) -> Result<Tensor<R>> {
        let data: Vec<f64> = x.to_vec();
        let residuals = vec![
