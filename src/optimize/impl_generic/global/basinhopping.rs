...
use super::{clamp_to_bounds, TensorGlobalResult};
    ...
    F: Fn(&Tensor<R>) -> Result<f64>,
{
    ...
    for iter in 0..options.max_iter {
        ...
        let delta = client
            .mul_scalar(&delta_unscaled, step_size)
            .map_err(|e| OptimizeError::NumericalError {
                message: format!("basinhopping: delta - {}", e),
            })?;

        // Perturb current position
        let x_perturbed_unclamped = client.add(&x_current, &delta).map_err(|e| {
            OptimizeError::NumericalError {
                message: format!("basinhopping: perturb - {}", e),
            }
        })?;

        let x_perturbed = clamp_to_bounds(client, &x_perturbed_unclamped, lower_bounds, upper_bounds)?;
