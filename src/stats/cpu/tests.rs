...
use crate::stats::{Binomial, ContinuousDistribution, DiscreteDistribution, Normal, Uniform};
...
fn test_2d_tensor_batch() {
    ...
    // Test with 2D tensor
    let x = Tensor::<CpuRuntime>::from_slice(&[-1.0, 0.0, 1.0, -2.0, 0.0, 2.0], &[2, 3], &device);
